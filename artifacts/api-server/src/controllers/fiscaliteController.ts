...
export async function getExportPpsiPdf(req: Request, res: Response): Promise<void> {
  try {
    const cooperativeId = req.user?.cooperativeId;
    if (!cooperativeId) { res.status(401).json({ erreur: "Coopérative non associée au compte" }); return; }
    const mois = parseInt(String(req.params["mois"]), 10);
    const annee = parseInt(String(req.params["annee"]), 10);
    if (mois < 1 || mois > 12 || annee < 2000) { res.status(400).json({ error: "mois et annee valides requis" }); return; }
    const pdf = await svc.genererPpsiPdf(cooperativeId, mois, annee);
    res.setHeader("Content-Type", "application/pdf");
    res.setHeader("Content-Disposition", `attachment; filename="ppsi-${annee}-${String(mois).padStart(2, "0")}.pdf"`);
    res.send(pdf);
  } catch (err) {
    req.log.error({ err }, "getExportPpsiPdf");
    res.status(500).json({ error: "Erreur serveur" });
  }
}
...
const VALID_TYPE_TAXE   = ["cnps","its","tva","impot_societes","taxe_apprentissage","fpc","tse","ppsi","autre"];
