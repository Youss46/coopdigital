    ...
    <Link href={href}>
      <div className={`bg-white rounded-xl border p-5 flex flex-col gap-3 cursor-pointer hover:shadow-md transition-shadow ${alert ? "border-red-200" : ok ? "border-green-200" : "border-gray-100"}`}>
        ...
        <div className="min-w-0">
          ...
          <p
            className={`text-sm sm:text-xl font-bold leading-tight break-words ${alert ? "text-red-700" : "text-gray-900"}`}
            title={value}
          >
            {value}
          </p>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          onClick={() => refetch()}
          disabled={isFetching}
          className="flex items-center justify-center gap-2 text-sm text-gray-500 hover:text-gray-800 border border-gray-200 rounded-lg px-3 py-2 transition-colors disabled:opacity-50 w-full sm:w-auto"
        >
