  ...
  return (
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Prévisions & Simulations</h1>
        ...
        {activeCampagneId && campagnes && (
          <select
            ...
            className="border rounded-lg px-3 py-2 text-sm bg-white shadow-sm w-full sm:w-auto"
          >
            {campagnes.map((c) => (
      ...
      {/* Tabs */}
      <div className="border-b flex gap-1 overflow-x-auto -mx-4 px-4 sm:mx-0 sm:px-0">
        {TABS.map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`px-3 sm:px-4 py-2.5 text-sm font-medium border-b-2 transition-colors relative whitespace-nowrap shrink-0 ${
            ...
            <div className="space-y-2">
              {cols.map(({ l, v, ecart }) => (
                <div key={l} className="flex items-center justify-between gap-3">
                  <span className="text-sm text-gray-600 shrink-0">{l}</span>
                  <span className="flex items-center gap-2 text-sm font-semibold text-gray-900 text-right flex-wrap justify-end">
                    <span className="whitespace-nowrap">{v}</span> {ecart !== undefined && ecartBadge(ecart)}
                  </span>
                </div>
              ))}
            </div>
