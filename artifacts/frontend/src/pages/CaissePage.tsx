...
function EtatCaisses({ caisses, loading, refetch, onJournal }: {
  ...
  const peutEcrire = ["pca", "directeur", "comptable", "caissier", "delegue"].includes(role);
    ...
    return (
      <div key={c.id} className={`bg-white rounded-xl border-2 p-5 shadow-sm ${sousMin ? "border-red-200" : ouvert ? "border-green-200" : "border-gray-100"}`}>
        ...
        <div className="mb-3 min-w-0">
          <p className="text-2xl font-bold text-gray-900 truncate">{FCFA(solde)}</p>
        ...
        {min > 0 && (
          <div className="mb-3">
            ...
            <p className={`text-xs mt-1 leading-tight ${sousMin ? "text-red-600 font-medium" : "text-gray-400"}`}>
              {sousMin
                ? `⚠️ Sous le minimum (${FCFA(min - solde)})`
                : `${FCFA(solde - min)} au-dessus du min.`}
            </p>
          </div>
        )}
